...
The build plan that used to live here — a phase-by-phase file list and a
week-by-week estimate — described work that is finished. See **Implementation
Status** at the top of this document for what shipped, and
[`packages/zest-ui/CLAUDE.md`](packages/zest-ui/CLAUDE.md) for the recipe used to
build it.
...
The component porting recipe, the store pattern, the animation contract, and the
list of gotchas already solved live in [`packages/zest-ui/CLAUDE.md`](packages/zest-ui/CLAUDE.md).
That file is the single source of truth for how a component gets built; this
document describes what the library *is*, not how to extend it.
