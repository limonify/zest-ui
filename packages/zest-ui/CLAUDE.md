...
- **The example app does run on an iOS simulator here** — `cd apps/example && npx expo start --ios --port 8082` (8081 is often taken by a sibling project). It loads through Expo Go, so the app's `react-native-gesture-handler` must match the version Expo Go ships: `npx expo install --check` tells you which. A mismatch fails at gesture attach with `undefined is not a function` in `attachHandlers.ts`, not at bundle time.
