...
- **The hook API, not the `Gesture.*()` builder.** All five parts use `usePanGesture({ … })`; the builder is deprecated in gesture-handler 3 and no longer appears in `src`. Name changes to watch: `.withTestId(x)` → `testID: x`, `.enabled(x)` → `enabled: x`, `.runOnJS(true)` → `runOnJS: true`, `.onStart` → `onActivate`, `.onEnd` → `onDeactivate`, and `Gesture.Simultaneous(a, b)` → `useSimultaneousGestures(a, b)`. The peer range is `>=3.1.0` for the same reason: the hooks do not exist in 2.x, and the two gesture shapes cannot be composed with each other, so there is no version of this package that supports both.
...
- A gesture is invisible in the rendered tree, so tests reach it only through gesture-handler's registry, keyed by the config's `testID`. **The registry proves the gesture exists, not that it was attached**: under the hook API a gesture registers when it is CREATED, so `getByGestureTestId` finds one that never reached a `GestureDetector`. An existence check therefore passes with the wiring deleted — it was a real assertion under the builder, where `withTestId` registered at attach time, and became worthless in the move to hooks. Assert the behaviour instead. Both parts forward their own `testID` to it — that registry is only populated under a test env, and it is what makes the gesture testable for zest's consumers too.
...
- **The example app does run on an iOS simulator here** — `cd apps/example && npx expo start --ios --port 8082` (8081 is often taken by a sibling project). **It no longer loads through Expo Go.** Expo Go ships gesture-handler 2.x and this package now needs `>=3.1.0` for the hook API, so the example app needs a development build (`npx expo run:ios`). That is the price of leaving the deprecated builder, and it is the same trade the sibling `limonify/ui` playground already made. A mismatch fails at gesture attach with `undefined is not a function` in `attachHandlers.ts`, not at bundle time.
  `npx expo run:ios` (a dev build) does **not** work at present: `expo-modules-jsi` fails to compile under Xcode 26.3 with `type of expression is ambiguous without a type annotation` in `JavaScriptCodable+Date.swift`. That is upstream's, not ours.
