...
- A gesture is invisible in the rendered tree, so tests reach it only through gesture-handler's registry, keyed by the config's `testID`. Both parts forward their own `testID` to it — that registry is only populated under a test env, and it is what makes the gesture testable for zest's consumers too.
