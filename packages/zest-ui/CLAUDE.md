...
- **Two runtime deps, and no more without a decision**: `@floating-ui/react-native` (positioning) and `react-native-gesture-handler` (a *required* peer at `>=2.32.0`, imported by `Slider.Control`, `Drawer.Popup`, `Drawer.SwipeArea`, `Toast.Root` and `NumberField.ScrubArea`). It was an optional peer until the root barrel made that untrue: `src/index.ts` re-exports every component and Metro does not tree-shake, so any root import pulls all five in. Only the subpath entries avoid it. `reanimated` is not used and is not planned — see the animation contract.
...
- **The `Gesture.*()` builder, not the hook API — deliberately, and this is the load-bearing decision in this section.** gesture-handler 3 deprecates the builder in favour of `usePanGesture({ … })`, and 0.10.0 moved to it. That cost the entire Expo default install base and has been reverted: Expo SDK 57 pins `react-native-gesture-handler` to `~2.32.0` (`bundledNativeModules.json`), the hooks do not exist in 2.x, so a `>=3.1.0` peer put this package outside every stock Expo project — `npm install` failed with `ERESOLVE`, `expo-doctor` warned on every run, and Expo Go could not load the app at all, because its binary embeds native 2.32 and a JS/native mismatch crashes at gesture attach.
  The builder is only `@deprecated`-annotated in 3.x, not removed, and it carries **no runtime warning**. It works on both versions end to end: `Gesture.Pan()` and `Gesture.Simultaneous()` are still exported, and 3.x's `GestureDetector` explicitly branches to the legacy detector for it — `if (props.gesture instanceof ComposedGesture || props.gesture instanceof BaseGesture)` in `v3/detectors/GestureDetector.js`. So the peer range is `>=2.32.0` and one code path serves 2.32 and 3.3 alike. **Do not "modernise" this to the hooks** until Expo ships a SDK whose pinned gesture-handler is 3.x; the deprecation has not landed and there is no deadline to beat.
  A single `GestureDetector` is locked to one gesture family — the branch above is per-detector — so a consumer's `simultaneousGesture` must be a builder gesture too. That is the one thing this costs, and it costs nothing in practice: the builder is available on the consumer's version whichever one they installed.
- Gesture callbacks touch React state, so they must set `runOnJS: true`.
...
- A gesture is invisible in the rendered tree, so tests reach it only through gesture-handler's registry, keyed by `withTestId`. **Under the builder the registry is populated at attach time**, so `getByGestureTestId` is a real assertion that a gesture reached a `GestureDetector` — which is what makes the `simultaneousGesture` "attaches the consumer gesture" tests in `SliderRoot.test.tsx` and `DrawerPopupSimultaneousGesture.test.tsx` meaningful: delete the `Gesture.Simultaneous` and they fail. (This is worth knowing because it is exactly what the hook API took away, registering on creation instead, which silently reduced both to existence checks that passed with the wiring gone. Restoring the builder restored the assertion.) Parts forward their own `testID` to it — that registry is only populated under a test env, and it is what makes the gesture testable for zest's consumers too.
...
- **The example app does run on an iOS simulator here** — `cd apps/example && npx expo start --ios --port 8082` (8081 is often taken by a sibling project). **It loads through Expo Go again** as of the return to the gesture builder: the app pins `react-native-gesture-handler` to `~2.32.0`, which is what SDK 57 bundles and what Expo Go's binary embeds, so JS and native agree. Keep that pin — bumping the example to 3.x is what broke Expo Go before, and it fails at gesture attach with `undefined is not a function` in `attachHandlers.ts`, not at bundle time, so a green bundle proves nothing.
  A development build (`npx expo run:ios`) is therefore no longer required, which is just as well: it does **not** work at present, because `expo-modules-jsi` fails to compile under Xcode 26.3 with `type of expression is ambiguous without a type annotation` in `JavaScriptCodable+Date.swift`. That is upstream's, not ours.
- **Most of the library still has not been seen on a screen.** A simulator run confirms mounting and layout; it does not confirm the things that need a finger or a screen reader. Say which of the two you did — never claim a device run that did not happen, and never let a successful launch stand in for an interaction you did not perform.

  What *has* been driven with a synthetic finger on an iPhone 16e simulator, through `apps/example`'s `VerifySection`: arrow centring under its trigger; RTL alignment and the RTL slider's fill direction (which is how the `SliderThumb`/`SliderIndicator` physical-`left` bug was found); `thumbCollisionBehavior` `push` (the crossed thumb is carried along, `minStepsBetweenValues` preserved) and `swap` (the crossed thumb stays put and the values re-sort); `Combobox.Trigger` + `Group` + `GroupLabel` + `Collection` + `Status`, and that choosing an item closes the list and it does **not** reopen when the Modal hands focus back to the input; `Drawer.Indent` scaling behind a transparent Modal; `Field`/`Form` validation and server errors. Snap points and VoiceOver announcements remain unverified.

  To drive it: `cliclick` for touches, `xcrun simctl io booted screenshot` to read the result. Three things cost real time — **`osascript -e 'tell application "Simulator" to activate'` before every gesture**, or the event is silently dropped; **`dm:` (drag-move), not `m:`**, or a pan never activates and only the initial press registers; and a press held past ~500 ms reads as a long press, so `onPress` never fires (`w:60` between `dd:` and `du:`). Map image pixels to screen points from the window's own geometry — `osascript … get {position, size} of window 1` — and re-read it after every relaunch, because the window moves.
